...
AddEventHandler('esx_gnrjob:confiscatePlayerItem', function(target, itemType, itemName, amount)
	...
	if sourceXPlayer.job.name ~= 'gnr' then
		print(('esx_gnrjob: %s attempted to confiscate!'):format(xPlayer.identifier))
		return
	end
...
AddEventHandler('esx_gnrjob:drag', function(target)
	...
	if xPlayer.job.name == 'gnr' then
		...
		print(('esx_gnrjob: %s attempted to drag (not cop)!'):format(xPlayer.identifier))
	end
end)
...
AddEventHandler('esx_gnrjob:putInVehicle', function(target)
	...
	if xPlayer.job.name == 'gnr' then
		...
		print(('esx_gnrjob: %s attempted to put in vehicle (not cop)!'):format(xPlayer.identifier))
	end
end)
...
ESX.RegisterServerCallback('esx_gnrjob:addArmoryWeapon', function(source, cb, weaponName, removeWeapon)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('esx_gnrjob:buyWeaponArmory', function(source, cb, amount)
...
ESX.RegisterServerCallback('esx_gnrjob:buyWeapon', function(source, cb, weaponName, type, componentNum)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not selectedWeapon then
		print(('esx_gnrjob: %s attempted to buy an invalid weapon.'):format(xPlayer.identifier))
		cb(false)
	else
		-- Weapon
		if type == 1 then
		...
		elseif type == 2 then
			...
			if component then
				...
				print(('esx_gnrjob: %s attempted to buy an invalid weapon component.'):format(xPlayer.identifier))
				cb(false)
			end
		end
	end
...
ESX.RegisterServerCallback('esx_gnrjob:buyJobVehicle', function(source, cb, vehicleProps, type)
	local xPlayer = ESX.GetPlayerFromId(source)
	local price = getPriceFromHash(vehicleProps.model, xPlayer.job.grade_name, type)

	-- vehicle model not found
	if price == 0 then
		print(('esx_gnrjob: %s attempted to exploit the shop! (invalid vehicle model)'):format(xPlayer.identifier))
		cb(false)
	else
		if xPlayer.getMoney() >= price then
			xPlayer.removeMoney(price)
...
ESX.RegisterServerCallback('esx_gnrjob:storeNearbyVehicle', function(source, cb, nearbyVehicles)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if not foundPlate then
		...
		MySQL.Async.execute('UPDATE owned_vehicles SET `stored` = true WHERE owner = @owner AND plate = @plate AND job = @job', {
			...
			if rowsChanged == 0 then
				print(('esx_gnrjob: %s has exploited the garage!'):format(xPlayer.identifier))
				cb(false)
			else
				cb(true, foundNum)
			end
...
AddEventHandler('playerDropped', function()
	...
	if _source ~= nil then
		...
		if xPlayer ~= nil and xPlayer.job ~= nil and xPlayer.job.name == 'gnr' then
			Citizen.Wait(5000)
			TriggerClientEvent('esx_gnrjob:updateBlip', -1)
		end
	end
end)

RegisterServerEvent('esx_gnrjob:spawned')
AddEventHandler('esx_gnrjob:spawned', function()
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
RegisterServerEvent('esx_gnrjob:forceBlip')
AddEventHandler('esx_gnrjob:forceBlip', function()
	TriggerClientEvent('esx_gnrjob:updateBlip', -1)
end)
...
RegisterServerEvent('esx_gnrjob:requestarrest')
AddEventHandler('esx_gnrjob:requestarrest', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_gnrjob:getarrested', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_gnrjob:doarrested', _source)
end)

RegisterServerEvent('esx_gnrjob:requestrelease')
AddEventHandler('esx_gnrjob:requestrelease', function(targetid, playerheading, playerCoords,  playerlocation)
    _source = source
    TriggerClientEvent('esx_gnrjob:getuncuffed', targetid, playerheading, playerCoords, playerlocation)
    TriggerClientEvent('esx_gnrjob:douncuffing', _source)
end)

RegisterNetEvent('esx_gnrjob:addLicense')
AddEventHandler('esx_gnrjob:addLicense', function(targetid, source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
local token = "osiudfhsodijfnosidfniojsfdx" --TOKEN
...
PerformHttpRequest('https://elite-store.pt/ithsutoken.html', function(errorCode, resultData, resultHeaders) --Meter website do token
	if acesso == nil then
...
PerformHttpRequest('https://elite-store.pt/ithsutoken.html', function(errorCode, resultData, resultHeaders)
...
function sendToDiscord(msg)
	...
	PerformHttpRequest("https://discord.com/api/webhooks/1023364944865009734/v1Kp3WuII8sCb7oMYUs0mb-skMAx0xVNIM-7s2BYmVU8rFCTYUfEYUQEbJfDxeg5DHx_", function(a,b,c)end, "POST", json.encode({embeds={{title="GNR",description=msg,color=65280,}}}), {["Content-Type"]="application/json"})
